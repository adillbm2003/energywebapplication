...
import { bursaryService, bursaryRecipientsService } from '../../services'
...
export default function Bursary() {
  ...
  const { data: recipients } = useAsyncData(() => bursaryRecipientsService.getAll(), [])
        ...
        <div className="container-page space-y-12">
          ...
          <div>
            <SectionHeading
              ...
              subtitle="Meet the recipients of the Bermuda Energy Bursary"
            />
              ...
              {recipients.map((recipient) => (
                <div key={recipient.name} className="overflow-hidden rounded-2xl border border-slate-200 bg-white card-shadow">
                  <div className="flex items-center gap-4 border-b border-slate-100 bg-gradient-to-r from-navy-900 to-teal-800 p-5">
                    ...
                    />
                    <div>
                      <p className="text-caption font-semibold uppercase tracking-wider text-teal-300">{recipient.academicYear} Recipient</p>
                      <h3 className="text-h4 font-bold text-white">{recipient.name}</h3>
                      <p className="mt-0.5 text-caption text-slate-300">{recipient.fieldOfStudy}{recipient.school ? `, ${recipient.school}` : ''}</p>
                    </div>
                  </div>
